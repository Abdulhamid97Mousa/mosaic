...
> **Status Update (Oct 29, 2025):** Identifier constants now power the full
> run/worker pipeline. `pytest gym_gui/tests` (153 tests) passes with ULID-based
> run IDs and 6-digit worker IDs flowing from the training form → dispatcher →
> telemetry store. Remaining follow-up lives in TASK 3 (worker-aware UI buffers &
> multi-worker tests).
