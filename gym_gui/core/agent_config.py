...
class BDIAgentConfig(AgentConfig):
    """Configuration for SPADE-BDI based agents.
    
    BDI (Belief-Desire-Intention) agents use SPADE framework with:
    - Beliefs: Agent's knowledge (position, goal, obstacles, visited cells)
    - Desires: Goals/objectives (reach goal, explore)
    - Intentions: Active plans/actions being executed
    """
    ...
    def get_telemetry_schema(self) -> Dict[str, Any]:
        """BDI telemetry schema includes agent state (beliefs/desires/intentions)."""
        return {
            "core_metrics": [
                "episode",
                "step",
    ...
    def get_required_fields(self) -> Set[str]:
        """BDI agents must provide core metrics."""
        return {
            "episode",
            "step",
            "reward",
    ...
    def get_optional_fields(self) -> Set[str]:
        """BDI agents may provide beliefs, desires, intentions, positions."""
        return {
            "beliefs",
            "desires",
            "intentions",
