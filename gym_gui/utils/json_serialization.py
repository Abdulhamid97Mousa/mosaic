...
def dumps(value: Any) -> bytes:
    ...
    try:
        prepared = _to_json_compatible(value)
...
def loads(payload: bytes | bytearray | str | memoryview) -> Any:
    ...
    return _from_json_compatible(decoded)
...
def _to_json_compatible(obj: Any) -> Any:
    ...
    if isinstance(obj, tuple):
        return {"__type__": "tuple", "data": [_to_json_compatible(item) for item in obj]}
    if isinstance(obj, Set):
        return {"__type__": "set", "data": [_to_json_compatible(item) for item in obj]}
...
def _from_json_compatible(obj: Any) -> Any:
    ...
    if isinstance(obj, Mapping):
        ...
        if marker == "ndarray":
            ...
            return array.reshape(shape)
        if marker == "tuple":
            return tuple(_from_json_compatible(item) for item in obj["data"])
        if marker == "set":
            return {_from_json_compatible(item) for item in obj["data"]}
        if marker == "float":
            value = obj.get("value", "nan")
            return float(value)
        return {key: _from_json_compatible(value) for key, value in obj.items()}
