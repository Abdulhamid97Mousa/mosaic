...
Example usage:
    ...
    llm-worker --run-id test123 --env babyai --task BabyAI-GoToRedBall-v0

    # Using specific model via OpenRouter
    llm-worker --run-id test123 --model anthropic/claude-3.5-sonnet

    # Using local vLLM
    llm-worker --run-id test123 --client vllm --model meta-llama/Llama-3.1-8B-Instruct

    # Load from config file
    llm-worker --config config.json
"""
...
from llm_worker import __version__
...
Examples:
  ...
  llm-worker --run-id test456 --env minihack --task MiniHack-Room-5x5-v0 \\
  ...
  llm-worker --run-id test789 --env crafter --agent-type cot
        """,
    )
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"llm-worker {__version__}",
    )
...
def build_config_from_args(args: argparse.Namespace) -> LLMWorkerConfig:
    """Build LLMWorkerConfig from parsed arguments."""
    # If config file provided, load it
    if args.config:
        config = LLMWorkerConfig.from_json_file(args.config)
...
def main(argv: list[str] | None = None) -> int:
    """Main entry point for llm-worker CLI.
    ...
    try:
        ...
        if args.interactive:
            ...
            from llm_worker.runtime import LLMWorkerRuntime
