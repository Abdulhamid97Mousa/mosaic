"""Command-line interface for VLM Worker.

Entry point: vlm-worker
...
Example usage:
    ...
    vlm-worker --run-id test123 --model anthropic/claude-3.5-sonnet

    # Using local vLLM
    vlm-worker --run-id test123 --client vllm --model meta-llama/Llama-3.1-8B-Instruct

    # Load from config file
    vlm-worker --config config.json
"""
...
from vlm_worker import __version__
from vlm_worker.config import (
    AGENT_TYPES,
    CLIENT_NAMES,
    ENV_NAMES,
    VLMWorkerConfig,
...
def create_parser() -> argparse.ArgumentParser:
    """Create argument parser for vlm-worker CLI."""
    parser = argparse.ArgumentParser(
        prog="vlm-worker",
        description="VLM Worker - Run Vision-Language Model agents on RL environments",
...
Examples:
  ...
  vlm-worker --run-id test456 --env minihack --task MiniHack-Room-5x5-v0 \\
  ...
  vlm-worker --run-id test789 --env crafter --agent-type cot
        """,
    )
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"vlm-worker {__version__}",
    )
    ...
    # VLM client settings
    vlm_group = parser.add_argument_group("VLM Client")
    vlm_group.add_argument(
        ...
        default="openrouter",
        dest="client_name",
        help="VLM client to use (default: openrouter). OpenRouter provides unified access to all major models.",
    )
    vlm_group.add_argument(
        "--model",
        type=str,
...
def build_config_from_args(args: argparse.Namespace) -> VLMWorkerConfig:
    """Build VLMWorkerConfig from parsed arguments."""
    # If config file provided, load it
    if args.config:
        config = VLMWorkerConfig.from_json_file(args.config)
...
def main(argv: list[str] | None = None) -> int:
    """Main entry point for vlm-worker CLI.
    ...
    logger.info(f"VLM Worker v{__version__}")
    logger.info(f"Run ID: {config.run_id}")
    logger.info(f"Environment: {config.env_name} / {config.task}")
    logger.info(f"VLM: {config.client_name} / {config.model_id}")
    ...
    try:
        ...
        if args.interactive:
            ...
            runtime = InteractiveVLMRuntime(config)
            runtime.run()
            logger.info("Interactive mode completed")
        else:
            # Autonomous mode - run all episodes
            from vlm_worker.runtime import VLMWorkerRuntime

            runtime = VLMWorkerRuntime(config)
            runtime.run()
            logger.info("VLM Worker completed successfully")
        return 0
    except KeyboardInterrupt:
        logger.warning("Interrupted by user")
        return 130
    except Exception as e:
        logger.exception(f"VLM Worker failed: {e}")
        return 1
