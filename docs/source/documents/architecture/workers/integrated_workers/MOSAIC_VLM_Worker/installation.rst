...
The MOSAIC VLM Worker is installed as an editable Python package from the
project's ``3rd_party/workers/mosaic/vlm_worker`` directory.
...
.. tabs::
   ...
   .. tab:: pip (API-only, recommended)
      ...
      .. code-block:: bash
         ...
         pip install -e 3rd_party/workers/mosaic/vlm_worker[all]
   ...
   .. tab:: pip (vLLM local inference)
      ...
      .. code-block:: bash
         ...
         pip install -e 3rd_party/workers/mosaic/vlm_worker[all,vllm]
...
.. code-block:: bash
   ...
   # Start the server with a vision-capable model:
   vllm serve Qwen/Qwen2.5-VL-3B-Instruct --api-key EMPTY
...
.. code-block:: bash
   ...
   export OPENROUTER_API_KEY=sk-or-v1-your_key_here
   vlm-worker --run-id verify-install \
       --env babyai \
