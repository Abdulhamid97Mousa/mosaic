...
- [ ] **CRITICAL BLOCKER**: Central async GUI bridge exists but `TelemetryAsyncHub` never starts event loop thread
...
### **What's Still Broken**
- ❌ **Agent tabs don't appear live** because the GUI side never attaches to live telemetry: `TelemetryAsyncHub.start()` creates tasks on a private loop that never runs, so no Qt signal is ever emitted for new runs.
- ❌ **No live streaming**: GUI polls for run status but never subscribes to `StreamRunSteps`/`StreamRunEpisodes`
- ❌ **Dispatcher never emits run_completed**: Tabs created but never cleaned up (signal exists but not wired)
...
graph LR
    subgraph "Service Bootstrap (bootstrap.py:10-95)"
        ...
        HUB_INIT -.->|"Comment: auto-start on subscribe"| NOTE[⚠️ Actually starts in<br/>subscribe_run Line 152-155]
    end
    ...
    subgraph "GUI Integration (main_window.py)"
        ...
        MW -->|Line 935-957| AUTO_SUB[_auto_subscribe_run<br/>⚠️ May not be called]
        AUTO_SUB -.->|Should call| SUBSCRIBE
    end
    ...
    style AUTO_SUB fill:#fbb,stroke:#333,stroke-width:3px
...
- ⚠️ **AsyncHub exists but subscribe mechanism unclear**. `subscribe_run()` will start the hub, but need to verify `_auto_subscribe_run()` is called (`main_window.py:935-957`)



### **Critical Path Forward**
1. **Start AsyncHub event loop** in dedicated thread (4 hours) - **THIS UNBLOCKS EVERYTHING**
...
- **Remaining blocker:** Verify `subscribe_run()` is actually called (check `_auto_subscribe_run()` or manual trigger)
...
- [ ] Verify `subscribe_run()` is called when training starts
...
### `gym_gui/ui/widgets/agent_loadout_dialog.py`
