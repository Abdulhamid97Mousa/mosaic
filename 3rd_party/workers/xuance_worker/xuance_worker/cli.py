...
def parse_args(args: list[str] | None = None) -> argparse.Namespace:
    ...
    # Action masking arguments (mirrors CleanRL --mask-invalid-actions)
    parser.add_argument(
        "--mask-invalid-actions",
        action="store_true",
        help="Mask invalid actions during training to speed up learning. "
             "Masks noop/toggle/done (actions 0,6,7) by default. "
             "Policy still outputs full action space (clean deployment).",
    )
    parser.add_argument(
        "--invalid-actions",
        type=str,
        default="0,6,7",
        help="Comma-separated action indices to mask (default: 0,6,7 = noop,toggle,done)",
    )
...
def main(args: list[str] | None = None) -> int:
    ...
    # Action masking: set env vars early so shims and environment pick them up
    if parsed.mask_invalid_actions:
        os.environ["MOSAIC_MASK_INVALID_ACTIONS"] = "1"
        os.environ["MOSAIC_INVALID_ACTIONS"] = parsed.invalid_actions
        logger.info(
            "Action masking enabled: masking actions [%s]", parsed.invalid_actions
        )
