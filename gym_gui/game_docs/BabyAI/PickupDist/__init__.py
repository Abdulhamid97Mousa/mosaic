"""Documentation for BabyAI PickupDist environment."""
from __future__ import annotations


def get_pickup_dist_html(env_id: str = "BabyAI-PickupDist-v0") -> str:
    """Generate PickupDist HTML documentation."""
    return (
        f"<h2>{env_id}</h2>"
        "<p>Locate and collect a specified object in a room with distractors. "
        "Target identified by type, color, or combination.</p>"
        "<h4>Mission</h4>"
        "<p><em>\"pick up a/the {color}/{type}/{color} {type}\"</em> — colors: red, green, blue, purple, yellow, grey; "
        "types: ball, box, key</p>"
        "<h4>Registered Variants</h4>"
        "<ul>"
        "<li><strong>BabyAI-PickupDist-v0</strong></li>"
        "<li><strong>BabyAI-PickupDistDebug-v0</strong>: Debug version</li>"
        "</ul>"
        "<h4>Observation</h4>"
        "<p>Dict with <code>image</code> (7×7×3 uint8), <code>direction</code> (0-3), and <code>mission</code> text.</p>"
        "<h4>Action Space (Discrete(7))</h4>"
        "<ul>"
        "<li>0 → turn left</li>"
        "<li>1 → turn right</li>"
        "<li>2 → move forward</li>"
        "<li>3 → pick up object</li>"
        "<li>4 → drop</li>"
        "<li>5 → toggle</li>"
        "<li>6 → done</li>"
        "</ul>"
        "<h4>Rewards</h4>"
        "<ul>"
        "<li>Success → <code>1 - 0.9 * (step_count / max_steps)</code></li>"
        "<li>Failure → 0</li>"
        "</ul>"
        "<h4>Episode End</h4>"
        "<ul>"
        "<li>Termination: agent picks up the target object</li>"
        "<li>Truncation: max_steps timeout</li>"
        "</ul>"
        "<p><strong>Keyboard:</strong> ←/A, →/D, ↑/W, Space (pick up), E/Enter (toggle).</p>"
        "<p>See docs: <a href=\"https://minigrid.farama.org/environments/babyai/PickupDist/\">BabyAI PickupDist</a></p>"
    )


BABYAI_PICKUP_DIST_HTML = get_pickup_dist_html()

__all__ = ["BABYAI_PICKUP_DIST_HTML", "get_pickup_dist_html"]
