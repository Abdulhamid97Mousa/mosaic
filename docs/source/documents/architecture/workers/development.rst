...
Use this checklist to verify your worker is complete:

- ☐ ``config.py`` implements ``run_id``, ``seed``, ``to_dict()``, ``from_dict()``
- ☐ ``runtime.py`` emits ``run_started``, ``heartbeat``, ``run_completed``/``run_failed``
- ☐ ``__init__.py`` exports ``get_worker_metadata()``
- ☐ ``pyproject.toml`` has ``[project.entry-points."mosaic.workers"]``
- ☐ ``requirements/myworker.txt`` exists
- ☐ Main ``pyproject.toml`` has ``myworker`` in ``[project.optional-dependencies]``
- ☐ ``pip install -e 3rd_party/my_worker`` succeeds
