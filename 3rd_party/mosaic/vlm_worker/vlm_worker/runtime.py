"""Runtime for VLM Worker.

This module provides the main episode loop that:
1. Creates VLM environments and agents
...
from vlm_worker.analytics import write_analytics_manifest
...
class VLMTelemetryEmitter:
    """Emits VLM-specific telemetry to JSONL files and stdout.
...
def _setup_api_key_env(config: VLMWorkerConfig) -> None:
...
class VLMWorkerRuntime:
    """Main runtime for running VLM agents on RL environments."""

    def __init__(self, config: VLMWorkerConfig) -> None:
        self.config = config
        self.telemetry = VLMTelemetryEmitter(
    ...
    def _create_agent(self) -> Any:
        ...
        from vlm_worker.agents import AgentFactory
    ...
    def _create_env(self) -> Any:
        ...
        Uses our own environment wrapper (vlm_worker.environments)
        which fixes compatibility issues with standard Gymnasium environments.
        """
        from vlm_worker.environments import make_env
        from omegaconf import OmegaConf
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if self._lifecycle_emitter:
            self._lifecycle_emitter.run_started(
                ...
                constant=LOG_MOSAIC_WORKER_VLM_RUNTIME_STARTED,
            )
        ...
        try:
                ...
                try:
                    ...
                    if self._lifecycle_emitter and (episode_idx + 1) % 5 == 0:
                        self._lifecycle_emitter.heartbeat(
                            ...
                            constant=LOG_MOSAIC_WORKER_VLM_RUNTIME_STARTED,  # Using RUNTIME_STARTED for heartbeat
                        )
            ...
            try:
                manifest_path = write_analytics_manifest(
                    self.config,
                    notes=f"VLM run with {total_episodes} episodes completed",
                )
            ...
            if self._lifecycle_emitter:
                self._lifecycle_emitter.run_completed(
                    summary,
                    constant=LOG_MOSAIC_WORKER_VLM_RUNTIME_STOPPED,
                )
            ...
            if self._lifecycle_emitter:
                self._lifecycle_emitter.run_failed(
                    {"error": str(e)},
                    constant=LOG_MOSAIC_WORKER_VLM_RUNTIME_ERROR,
                )
            self.telemetry.close()
            raise
    ...
    def _obs_to_str(self, obs: Any) -> str:
        ...
        if self.config.env_name in ("multigrid", "mosaic_multigrid") and hasattr(obs, "shape"):
            from vlm_worker.environments import generate_multigrid_description
            import numpy as np
    ...
    def _handle_reset(self, seed: Optional[int] = None) -> None:
        ...
        try:
            ...
            if self.config.env_name in ["babyai", "minigrid"]:
                ...
                # MOSAIC MultiGrid extension
                from vlm_worker.environments import get_multigrid_instruction_prompt
    ...
    def _handle_step(self) -> None:
        ...
        try:
            ...
            try:
                ...
                if _HAS_GYM_GUI and log_constant:
                    log_constant(logger, LOG_MOSAIC_WORKER_VLM_ACTION_DEFAULTED, extra={
                        "step_index": self._step_idx,
                        "error": str(e),
            ...
            if terminated or truncated:
                ...
                if self.config.env_name in ("babyai", "minigrid") and self._reset_seed is not None:
                    ...
                    if _HAS_GYM_GUI and log_constant:
                        log_constant(logger, LOG_MOSAIC_WORKER_VLM_EPISODE_AUTO_RESET, extra={
                            "episode_index": self._episode_idx,
