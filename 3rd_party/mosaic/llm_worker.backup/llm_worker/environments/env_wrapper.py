...
class EnvWrapper(gym.Wrapper):
    ...
    def _process_observation(self, obs):
        if self.env_name in ["nle", "minihack"]:
            obs = obs
        elif self.env_name in ["babyai", "minigrid"]:  # MiniGrid is same framework as BabyAI
            obs = obs
    ...
    def get_instruction_prompt(self, instructions=None):
        if self.env_name == "nle":
            from .nle import get_instruction_prompt

            return get_instruction_prompt()
        elif self.env_name == "minihack":
            from .minihack import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name in ["babyai", "minigrid"]:  # MiniGrid uses same prompts as BabyAI
            from .babyai_text import get_instruction_prompt

            return get_instruction_prompt(self.env, mission=instructions)
        elif self.env_name == "textworld":
            from .textworld import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name == "babaisai":
            from .babaisai import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name == "crafter":
            from .crafter import get_instruction_prompt
