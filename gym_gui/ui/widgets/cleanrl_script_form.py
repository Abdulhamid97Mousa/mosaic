"""CleanRL Custom Script form for launching bash-based training scripts.
...
_LOGGER = logging.getLogger("gym_gui.ui.cleanrl_script_form")
...
class CleanRlScriptForm(QtWidgets.QDialog):
    """Configuration dialog for CleanRL custom training scripts."""

    def __init__(self, parent: Optional[QtWidgets.QWidget] = None, **kwargs: Any) -> None:
        super().__init__(parent)
        self.setWindowTitle("CleanRL Custom Script")
    ...
    def _on_script_changed(self, index: int) -> None:
        ...
        if data == "BROWSE":
            initial_dir = str(CLEANRL_SCRIPTS_DIR) if CLEANRL_SCRIPTS_DIR.is_dir() else str(Path.home())
            path, _ = QFileDialog.getOpenFileName(
                self,
                "Select CleanRL Script",
                initial_dir,
    ...
    def _on_accept_clicked(self) -> None:
        ...
        run_name = _generate_run_id("cleanrl-script", script_name)
        run_id = str(ULID())
    ...
    def _build_config(self, script_path: str, run_id: str, run_name: str) -> Dict[str, Any]:
        ...
        worker_config: Dict[str, Any] = {
            "run_id": run_id,
            "algo": "ppo",  # default placeholder; script overrides via jq
        ...
        environment: Dict[str, Any] = {
            ...
            "CLEANRL_AGENT_ID": "cleanrl_agent",
            "TRACK_TENSORBOARD": "1" if track_tb else "0",
            "TRACK_WANDB": "1" if track_wandb else "0",
            "WANDB_MODE": "online" if track_wandb else "offline",
...
if not _factory.has_script_form("cleanrl_worker"):
    _factory.register_script_form(
        "cleanrl_worker",
        lambda parent=None, **kw: CleanRlScriptForm(parent=parent, **kw),
    )
