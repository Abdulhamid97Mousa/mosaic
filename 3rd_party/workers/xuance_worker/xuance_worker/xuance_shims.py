...
# ---------------------------------------------------------------------------
# Shim 4: Action masking via MOSAIC_MASK_INVALID_ACTIONS env var
# ---------------------------------------------------------------------------
# When MOSAIC_MASK_INVALID_ACTIONS=1, patch MARLAgents.__init__ so that
# config.use_actions_mask is forced to True before XuanCe reads it.
# This activates XuanCe's built-in avail_actions pipeline which reads
# buf_avail_actions from the vec-env and passes it through to the policy.
#
# The env itself (mosaic_multigrid.py) returns the real mask when this env
# var is set -- the two sides work together as a shim-layer-only solution
# with zero changes to the XuanCe library.
# ---------------------------------------------------------------------------

_ACTION_MASK_PATCHED = False


def _shim_action_masking() -> None:
    """Force config.use_actions_mask=True when MOSAIC_MASK_INVALID_ACTIONS=1."""
    global _ACTION_MASK_PATCHED
    if _ACTION_MASK_PATCHED:
        return
    ...
    try:
        from xuance.torch.agents.base.agents_marl import MARLAgents
    except ImportError:
        return

    _original_init = MARLAgents.__init__

    def _patched_init(self, config, *args, **kwargs):
        if not getattr(config, "use_actions_mask", False):
            config.use_actions_mask = True
        _original_init(self, config, *args, **kwargs)

    MARLAgents.__init__ = _patched_init
    _ACTION_MASK_PATCHED = True
    invalid = os.environ.get("MOSAIC_INVALID_ACTIONS", "0,6,7")
    LOGGER.info(
        "Shim applied: action masking enabled (masking actions [%s])", invalid
    )
