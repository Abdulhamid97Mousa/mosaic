"""Utilities for validating MARLlib worker configurations via dry-run."""
...
def run_marllib_dry_run(
    config_payload: Dict[str, Any],
    *,
    timeout_seconds: int = 120,
) -> Tuple[bool, str]:
    """Execute ``marllib_worker.cli`` in ``--dry-run`` mode to validate a payload.

    This function spawns a subprocess to validate the MARLlib worker configuration
    ...
    Args:
        config_payload: Trainer configuration dictionary generated by the MARLlib form.
            Should contain the worker config with algo, environment_name, map_name, etc.
    ...
    Example:
        ...
        ...     "map_name": "3m",
        ... }
        >>> success, output = run_marllib_dry_run(config)
        >>> if success:
    ...
    with tempfile.TemporaryDirectory(prefix="marllib-dry-run-") as tmp_dir:
        ...
        try:
            ...
            return False, output or "MARLlib dry-run timed out"

    output = _merge_output(result.stdout, result.stderr)
    success = result.returncode == 0
    if not success and not output:
        output = "MARLlib dry-run failed without output."
    return success, output


__all__ = ["run_marllib_dry_run"]
