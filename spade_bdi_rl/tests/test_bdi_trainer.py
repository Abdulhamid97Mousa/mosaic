...
from ..core import BDITrainer, RunConfig, TelemetryEmitter
from .. import worker
from gym_gui.config.game_configs import FrozenLakeConfig
from ..worker import BDITrainer as WorkerBDITrainer
from .. import worker
...
class TestBDITrainerInstantiation:
    ...
    def test_bdi_trainer_with_frozenlake_v2(self, run_config: RunConfig, emitter: TelemetryEmitter):
        ...
        game_config = FrozenLakeConfig(grid_height=5, grid_width=5)
        adapter = create_adapter("FrozenLake-v2", game_config=game_config)
        trainer = BDITrainer(
            adapter,
            run_config,
            emitter,
        ...
        assert trainer.config.game_id == "FrozenLake-v2"
    ...
    def test_bdi_trainer_with_cliffwalking(self, run_config: RunConfig, emitter: TelemetryEmitter):
        ...
        run_config.game_id = "CliffWalking-v1"
...
class TestBDITrainerConfiguration:
    ...
    def trainer(self, tmp_path: Path) -> BDITrainer:
        ...
        game_config = FrozenLakeConfig(grid_height=4, grid_width=4)
        adapter = create_adapter("FrozenLake-v2", game_config=game_config)
    ...
    def test_episode_metadata_includes_bdi_fields(self, trainer: BDITrainer):
        ...
        # episode_index=0, episode_seed=42 (unique seed per episode for environment variation)
        # seed=100 (config.seed for reproducibility)
        ...
        assert metadata["seed"] == 100  # Base seed from config
        assert metadata["episode_seed"] == 42  # Unique seed per episode
