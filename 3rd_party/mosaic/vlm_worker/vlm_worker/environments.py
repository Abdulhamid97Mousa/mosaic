"""Environment wrappers for VLM Worker.
...
def generate_multigrid_description(
    ...
    """
    # Import MOSAIC extension (now in vlm_worker.environments.multigrid)
...
def make_env(
    ...
    if env_name in ("babyai", "minigrid"):
        ...
        # Now wrap with text wrapper (local vlm_worker version)
        from vlm_worker.environments.babyai_text import BabyAITextCleanLangWrapper
        ...
        # For other environments, use vlm_worker's make_env
        from vlm_worker.environments import make_env as vlm_worker_make_env
        return vlm_worker_make_env(env_name, task, config, render_mode=render_mode)
