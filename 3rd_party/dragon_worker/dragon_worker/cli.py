"""Command-line interface for Dragon Worker.

Usage:
    dragon-worker --config config.json
    dragon-worker --difficulty medium --run-id test_run
    dragon-worker --interactive
"""
...
from .runtime import DragonWorkerRuntime
...
def main() -> int:
    """Main entry point for dragon-worker CLI.
    ...
    parser = argparse.ArgumentParser(
        prog="dragon-worker",
        description="Komodo Dragon chess engine worker for MOSAIC operators",
    )
    ...
    parser.add_argument(
        "--skill-level",
        type=int,
        help="Dragon skill level (1-25, overrides difficulty)",
    )
    parser.add_argument(
        "--depth",
        type=int,
    ...
    parser.add_argument(
        "--dragon-path",
        type=str,
        help="Path to Dragon binary (auto-detected if not provided)",
    )
    parser.add_argument(
        "--interactive", "-i",
        action="store_true",
    ...
    logger = logging.getLogger("dragon_worker.cli")
    ...
    try:
        ...
        if args.interactive:
            ...
            with runtime:
                ...
                logger.info("Dragon engine ready")
                print(json.dumps({
                    "status": "ready",
                    "config": config.to_dict(),
                    "dragon_path": runtime._dragon_path,
                }))
