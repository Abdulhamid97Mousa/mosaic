...
3. **Action + step telemetry missing**  ▸ **Priority: High**
   - **Risk:** `SessionController.step_processed` only forwards render payloads; the log pane drops action IDs, rewards, and timers once new sessions start. Without action traces we cannot reproduce an agent’s decision trail or train offline models.
   - **Contrarian question:** *Isn’t the console log enough for now?* Console output is lossy, unstructured, and vanishes after restart. Researchers need machine-readable traces.
   - **Recommendation:** Introduce a structured `StepRecord` model and log writer; gate adapter PRs on emitting action metadata.

4. **Directory layout lacks a writable `var/` tier**  ▸ **Priority: Medium**
...
5. **No Actor abstraction across control modes**  ▸ **Priority: High**
   - **Risk:** `SessionController` hard-codes turn strings (“human”, “agent”) and funnels both human and automatic actions through `_apply_action`; `perform_agent_step()` simply calls `_select_agent_action()` which samples directly from the Gym action space. `HumanInputController` only emits keyboard-driven slots, so swapping in an RL policy means editing `SessionController`, the presenter, and the adapter signatures together. Day 4’s actor sketch (§6.2, §12) assumed a polymorphic `Actor` boundary—we still haven’t created it.
   - **Contrarian question:** *Do we really need an actor layer while shipping human-only demos?* Yes—`ControlMode.AGENT_ONLY` already exists, yet there is no way to inject an agent implementation besides patching the controller. Hybrid and co-op modes from Day 4 demand more than the two hard-coded strings; without an actor protocol we can’t reconcile human inputs, scripted bots, and future multi-agent coordination.
   - **Recommendation:** Define an `Actor` protocol (e.g., `select_action(observation)`, `on_step(step_record)`) with concrete implementations for human keyboard, random agent, scripted agent, RL policy. Refactor `SessionController.perform_agent_step()` to request actions from the active actor registry, allowing `HumanInputController` to become just one actor implementation instead of the only path.

6. **Static-analysis debt piling up (cyclomatic, dead stores, asset dispatch)** ▸ **Priority: Medium**
...
| **Panel wiring cleanup** (§3, §5) called for presenter-driven signal maps and removal of inline Qt hookups. | `MainWindow._connect_signals()` still binds dozens of slots manually; presenter only re-emits a subset. | Signal sprawl causes duplicated state management (e.g., awaiting-human logic). | Extend presenter to own ControlPanel signal hookups and remove duplicated wiring in `MainWindow`. |
...
### What we have
`gym_gui.services` is still empty, leaving orchestration, logging, and persistence tangled inside `SessionController`. Cura’s catalog of services (e.g., `CuraApplication`, `TaskManagement`, `Backups`) shows how we can separate responsibilities, and the fbs tutorial highlights how an application context can own resource lookups.

### Immediate services to define
- **ActorService** — registers actors (`HumanKeyboardActor`, `RandomAgentActor`, `PolicyActor`), exposes `next_action(session_state)`, and keeps Day 4 control-mode promises alive.
- **StorageRecorderService** — maintains the in-memory ring buffer, writes frames/telemetry into `var/records`, emits `frame_ref` for every `AdapterStep`, and mirrors Cura’s auto-save safety net with pruning hooks.
- **TelemetryService** — builds `StepRecord` objects, persists NDJSON, and publishes model updates so the UI can switch to a `QTableView` without tight coupling.
...
| --- | --- | --- | --- | --- |
| **Phase 0 – Hardening** | Week 0 (now) | • Restructure Day 4 doc per Day 0–3 style.<br>• Add lint fixes (unused imports, dead variables). | Docs: Zahra<br>Code: Hamid | ✅ `ruff --select F` passes.<br>✅ Day 4 rewrite approved in review. |
| **Phase 1 – Rendering split** | Weeks 1–2 | • `IRendererStrategy` live with grid + RGB implementations.<br>• Render-mode negotiation enforced in unit test.<br>• Prototype LunarLander adapter feeding RGB frames. | Hamid + Ayesha | ✅ Taxi + LunarLander demo toggles renderers without UI crashes.<br>✅ Renderer tests run under `pytest -k rendering`. |
| **Phase 2 – Storage recorder** | Weeks 3–4 | • `var/` scaffolding with pruning CLI.<br>• `StorageRecorder` writing ring buffer + PNG disk snapshots.<br>• `AdapterStep.frame_ref` populated for Taxi + LunarLander. | Storage: Bilal<br>Adapters: Hamid | ✅ 1 k-step LunarLander session keeps ≤512 frames in RAM, remainder on disk.<br>✅ `python -m gym_gui.tools.prune --dry-run` reports reclaimed bytes. |
| **Phase 3 – Telemetry model** | Weeks 5–6 | • `StepRecord` dataclass persisted as NDJSON.<br>• Log pane upgraded to `QTableView` with filters and export button.<br>• Offline replay CLI consumes stored traces. | UI: Zahra<br>Backend: Bilal | ✅ Exported NDJSON replayed via `python -m gym_gui.tools.replay`.<br>✅ Table view filters by action ID < 50 ms latency. |
| **Phase 4 – Scalability & QA** | Weeks 7–12 | • Optionally swap PNG writer for `.zarr` or Parquet pilot.<br>• Add headless regression suite (Taxi + LunarLander) using xvfb.<br>• Introduce metrics dashboard (disk usage, frame write rate). | QA: Ayesha<br>Infra: Bilal | ✅ CI job records headless run artifacts.<br>✅ Metrics panel warns when disk quota ≥80 %. |
