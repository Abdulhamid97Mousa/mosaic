"""
Curriculum Training Module for XuanCe.

This module provides a unified training loop that uses Syllabus-RL for
curriculum learning with XuanCe algorithms. Unlike the bash-based phase scripts,
this runs everything in a SINGLE PROCESS, preserving neural network weights
...
class CurriculumTrainingConfig:
    ...
    @classmethod
    def from_worker_config(cls, config: XuanCeWorkerConfig) -> "CurriculumTrainingConfig":
        """Create CurriculumTrainingConfig from XuanCeWorkerConfig."""
        ...
        num_envs = config.parallels or algo_params.get("num_envs", extras.get("num_envs", 4))

        # XuanCe uses 'running_steps' instead of 'total_timesteps'
        total_timesteps = config.running_steps

        # Device handling - XuanCe uses "cuda" or "cpu" strings
        cuda = config.device.startswith("cuda") if config.device else True
...
class PPOAgent(nn.Module):
    ...
    def __init__(self, envs: gym.vector.VectorEnv):
        super().__init__()
        # Get observation and action spaces from VectorEnv
...
def run_curriculum_training(config: CurriculumTrainingConfig) -> Dict[str, Any]:
    ...
    Note:
        FastLane environment variables (GYM_GUI_FASTLANE_*, XUANCE_RUN_ID, etc.)
    ...
    _LOGGER.info(
        ...
        os.environ.get("XUANCE_RUN_ID", os.environ.get("CLEANRL_RUN_ID", "not set")),
    )
    ...
    # NOTE: apply_wrappers=True to ensure ImgObsWrapper + FlattenObservation
    # are applied. sitecustomize.py is not guaranteed to be loaded.
...
def is_curriculum_config(config: XuanCeWorkerConfig) -> bool:
