...
class EnvWrapper(gym.Wrapper):
    ...
    def get_instruction_prompt(self, instructions=None):
        if self.env_name == "nle":
            from vlm_worker.environments.nle import get_instruction_prompt

            return get_instruction_prompt()
        elif self.env_name == "minihack":
            from vlm_worker.environments.minihack import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name == "babyai":
            from vlm_worker.environments.babyai_text import get_instruction_prompt
            ...
            from vlm_worker.environments.textworld import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name == "babaisai":
            from vlm_worker.environments.babaisai import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name == "crafter":
            from vlm_worker.environments.crafter import get_instruction_prompt
