...
| --- | --- | --- |
| **1. Rendering locked to grids** | **Done – [2025-10-13]** Renderer registry + strategy layer now drive `RenderTabs`, with grid and RGB implementations resolved through the service locator. | Backfill regression tests for renderer negotiation, wire additional modes (ASCII, surface), and document extension points for new strategies. |
...
| **3. Lack of writable `var/` tier** | *Resolved ✅* – runtime artifacts moved to `gym_gui/var/`. | Bootstrap now targets `var/` (records, telemetry, cache, tmp); docs updated with cleanup guidance. |
...
- Packaged `runtime/data/toy_text` assets remain for toy-text adapters, while writable artifacts now flow into `gym_gui/var/` to avoid accidental commits and large repo churn.
...
## Storage & Telemetry – Resolved Issues

The following entries collect the Day 5 storage and telemetry fixes that landed in the same mitigation sprint, so the end-to-end pipeline stays traceable in one place.
...
Run these commands against the active telemetry database (e.g. `gym_gui/var/telemetry/telemetry.sqlite`):
...
path = "gym_gui/var/telemetry/telemetry.sqlite"
...
- ✅ Structured telemetry, actor orchestration, session-wide seeding, and the renderer strategy layer are complete and removed from the active Day 5 issue list (issue 1 closed 2025-10-13).
...
### 9.1 Bug #1: Start Game Freeze – Human Input Gated Inconsistently *(Status: Resolved ✅ )*
...
### 9.2 Bug #2: Taxi-v3 Parameter Crash *(Status: Resolved ✅ )*
...
### 9.3 Bug #3: LunarLander Continuous Mode – Shared Action Mapping Now Live *(Status: Resolved ✅ )*
...
### 9.4 Bug #4: Pause Button Doesn't Stop CarRacing (Idle Timer Issue) *(Status: Resolved ✅ )*
...
flowchart TD
    ...
    E -->|flush| F["JSONL files in gym_gui/var/records/"]
...
3. **`SessionRecorder`** (`gym_gui/storage/session.py`)
   ...
   - Buffers steps in memory until `finalize_episode()` is called
    - Writes to `gym_gui/var/records/{episode_id}/episode.jsonl`
...
- `gym_gui/var/records/` – Output directory for persisted episodes
...
- **Structured steps (`gym_gui/var/records/<episode_id>/episode.jsonl`):** newline-delimited JSON
    ...
    ```
- **SQLite telemetry (`gym_gui/var/telemetry/telemetry.sqlite`):** tables `steps` and `episodes`
...
### 11.1 Issue 1 – Rendering strategy layer in place *(Resolved 2025-10-13 ✅)*
- **What the code does:** `RenderTabs` now resolves renderers from a shared `RendererRegistry`, with `_RendererHost` instances delegating to strategy implementations (`gym_gui/rendering/interfaces.py`, `gym_gui/rendering/registry.py`, `gym_gui/rendering/strategies/grid.py`, `gym_gui/rendering/strategies/rgb.py`). Grid payloads flow through the legacy `GridRenderer` wrapped by `GridRendererStrategy`, while RGB frames render via a new QLabel-based strategy. Replay previews reuse the same registry to avoid duplicate logic.
- **Impact:** Grid and RGB render paths are decoupled from the widget, adapters advertise `supported_render_modes`, and adding new strategies now requires only registry registration. Manual QA confirms dynamic tab activation matches payload mode.
- **Follow-up:** Add unit/integration tests that cover renderer negotiation, implement additional strategies (ASCII, surface), and extend docs describing how to register custom renderers.
...
### 11.3 Issue 3 – Writable `var/` tier *(Resolved 2025-10-13 ✅)*
- **What the code does now:** `StorageRecorderService` writes episodes to `gym_gui/var/records/`, the telemetry bootstrap seeds `gym_gui/var/telemetry/telemetry.sqlite`, and the repo ships a documented `var/` scaffold (records, telemetry, cache, tmp).
- **Impact:** Generated files no longer mingle with packaged assets, reducing the risk of accidental commits and clarifying retention/cleanup policies.
- **Follow-up:** Consider a one-time migration helper that moves any legacy `runtime/data/episodes` runs into the new tree and exposes a CLI to prune/backup `var/` contents.
...
1. **Harden the renderer strategy layer** – Add regression tests, document extension points, and expand strategy coverage (ASCII, surface) so the new registry keeps scaling cleanly.
