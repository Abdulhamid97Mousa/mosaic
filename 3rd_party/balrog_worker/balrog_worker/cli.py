...
Entry point: balrog-worker

Example usage:
    # Using OpenRouter (default, supports all major models)
    export OPENROUTER_API_KEY=sk-or-...
    balrog-worker --run-id test123 --env babyai --task BabyAI-GoToRedBall-v0

    # Using specific model via OpenRouter
    balrog-worker --run-id test123 --model anthropic/claude-3.5-sonnet

    # Using local vLLM
    balrog-worker --run-id test123 --client vllm --model meta-llama/Llama-3.1-8B-Instruct

    # Load from config file
    balrog-worker --config config.json
"""
...
from balrog_worker import __version__
...
def create_parser() -> argparse.ArgumentParser:
    """Create argument parser for balrog-worker CLI."""
...
Examples:
  ...
  balrog-worker --run-id test456 --env minihack --task MiniHack-Room-5x5-v0 \\
  ...
  balrog-worker --run-id test789 --env crafter --agent-type cot
        """,
    )
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"balrog-worker {__version__}",
    )
...
def main(argv: list[str] | None = None) -> int:
    """Main entry point for balrog-worker CLI.
    ...
    logger = logging.getLogger("balrog_worker")
    ...
    try:
        ...
        if args.interactive:
            # Interactive mode for GUI step-by-step control
            from balrog_worker.runtime import InteractiveRuntime
            ...
            from balrog_worker.runtime import BarlogWorkerRuntime
