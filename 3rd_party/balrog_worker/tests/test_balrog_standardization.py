"""Tests for BALROG Worker standardization (Phase 4).
...
class TestPhase4_2_WorkerMetadata:
    """Test BALROG worker metadata and capabilities."""

    def test_get_worker_metadata_exists(self):
        """Test that get_worker_metadata function exists."""
        from balrog_worker import get_worker_metadata
    ...
    def test_worker_metadata_has_correct_fields(self):
        ...
        try:
            ...
            assert metadata.name == "BALROG Worker"
    ...
    def test_worker_capabilities_declares_correct_worker_type(self):
        """Test that WorkerCapabilities declares worker_type='balrog'."""
        from balrog_worker import get_worker_metadata

        try:
            metadata, capabilities = get_worker_metadata()

            assert capabilities.worker_type == "balrog"
        except ImportError:
            pytest.skip("gym_gui not available")
    ...
    def test_worker_capabilities_declares_env_families(self):
        ...
        try:
            ...
            # BALROG supports these env families
...
class TestPhase4_3_EntryPointDiscovery:
    ...
    def test_entry_point_registered(self):
        """Test that 'balrog' entry point is registered in mosaic.workers group."""
        try:
        ...
        # Check if 'balrog' is registered
        balrog_eps = [ep for ep in eps if ep.name == "balrog"]
        assert len(balrog_eps) > 0, "balrog entry point not found in mosaic.workers group"

        # Load the entry point
        balrog_ep = balrog_eps[0]
        get_metadata = balrog_ep.load()
        assert callable(get_metadata)

    def test_worker_discoverable_via_discovery_system(self):
        """Test that BALROG worker can be discovered via WorkerDiscovery."""
        try:
            ...
            # Check if balrog worker was discovered
            assert "balrog" in workers_dict or any(
                "balrog" in str(w).lower() for w in workers_dict.values()
            ), "BALROG worker not discovered"
        except ImportError:
            pytest.skip("gym_gui not available")
...
class TestPhase4_4_AnalyticsStandardization:
    ...
    def test_write_analytics_uses_worker_analytics_manifest(self):
        ...
        from balrog_worker.analytics import write_analytics_manifest
        from balrog_worker.config import BarlogWorkerConfig
    ...
    def test_analytics_manifest_has_standardized_structure(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                assert loaded.worker_type == "balrog"
                assert loaded.artifacts is not None
                assert loaded.metadata is not None
        except ImportError:
            pytest.skip("gym_gui not available")

    def test_analytics_manifest_stores_balrog_specific_metadata(self):
        """Test that manifest stores BALROG-specific metadata."""
        from balrog_worker.analytics import write_analytics_manifest
        from balrog_worker.config import BarlogWorkerConfig
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Check BALROG-specific metadata
...
class TestPhase4_5_ConfigLoading:
    ...
    def test_load_worker_config_from_direct_format(self):
        ...
        from balrog_worker.config import load_worker_config
...
class TestPhase4_6_AgentTypes:
    ...
    def test_valid_agent_types(self):
        ...
        from balrog_worker.config import BarlogWorkerConfig, AGENT_TYPES
