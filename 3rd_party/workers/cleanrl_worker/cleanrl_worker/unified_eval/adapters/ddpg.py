...
class DDPGSelector(BaseSelector):
    """ActionSelector for DDPG algorithm.

    Works with: ddpg_continuous_action
    """
    ...
    def __init__(
        ...
        """Initialize DDPG selector.

        Args:
            actor_cls: Optional Actor class.
            qf_cls: Optional QNetwork class (not used for eval, but needed for loading).
    ...
    def load(
        ...
        **kwargs: Any,
        ...
        """Load DDPG model from checkpoint.

        DDPG checkpoint format: tuple (actor_state_dict, qf_state_dict)
        ...
        if isinstance(checkpoint, tuple):
            ...
            # Handle single state_dict format
